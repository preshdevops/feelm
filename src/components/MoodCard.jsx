...
export default function MoodCard({ mood, isSelected, onClick }) {
  return (
    <button
      ...
      className={`flex items-center gap-3 px-5 py-3 border transition-all duration-200 cursor-pointer rounded-lg hover:scale-[1.02]
        ${isSelected 
    ...
    >
      <span className={isSelected ? 'text-accent' : 'text-cinema-500'}>
        {getMoodIcon(mood.id)}
      </span>
      <span className="font-body text-[13px] font-medium tracking-wide">{mood.label}</span>
    </button>
  );
}
